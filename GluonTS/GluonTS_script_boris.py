...
os.makedirs('progQ', exist_ok=True)
np.savetxt('progQ/Quants_'+'bd.txt', np.array([np.array(x) for x in quants_series]))

df1 = pd.read_csv('./data/electricity/datasets/mhlv/raw/MHLV.csv')
myda1=df1.to_numpy()
myda=myda1[:,1:]
...
for mode in modee:
    ...
    if(mode=='mlp'):
        ...
        # )
        estimator = SimpleFeedForwardEstimator(num_hidden_dimensions=[10], prediction_length=48, trainer=Trainer())
        ...
        estimator = NBEATSEnsembleEstimator(freq=freq, prediction_length=48, meta_bagging_size = 1,meta_context_length = [prediction_length * mlp for mlp in [7] ],meta_loss_function = ['sMAPE'], trainer=Trainer())
        ...
        #estimator =DeepAREstimator(prediction_length=48, freq=freq, trainer=Trainer())
        ...
        estimator =DeepAREstimator(prediction_length=48, freq=freq, 
                                   num_layers=2, 
                                   num_cells=256,
                                   dropout_rate=0.0,
                                   lags_seq=list(range(1, 169)),
                                   trainer=Trainer(ctx=mx.cpu(), epochs=25, learning_rate=1e-3)) #4.14
        ...
        estimator =LSTNetEstimator(prediction_length=48, context_length=48,num_series=55,skip_size=1,ar_window=1,channels=1, trainer=Trainer())
        ...
        estimator = DeepTPPEstimator(prediction_interval_length=48, context_interval_length=24, num_marks=1, freq=freq,
                                     trainer=Trainer(hybridize=False))  # needs additiona parameters prediction_interval_length context_interval_length num_marks

    elif(mode == 'check'):
        estimator =TemporalFusionTransformerEstimator(prediction_length=48, freq=freq, trainer=Trainer(epochs=5))#works
    ...
    for day in range(364,-1,-1):#for day in range(364,-1,-1):
        ...
        for q in range(0, len(quants_series[x])):
            ...
            if day==364:
                pr_q_all.append(pr_q)
            else:
                pr_q_all[q]=np.vstack((pr_q_all[q],pr_q))
